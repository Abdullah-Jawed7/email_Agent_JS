...
const Email_Writer = new Agent({
    ...
    model:CUSTOM_MODEL_PROVIDER.getModel()
})

const Email_Writer_and_sender = new Agent({
     name:'Email Writer and Sender',
    instructions:"First, invoke the Email_Writer to compose an email based on the user’s context. Then invoke the EmailSenderTool with parameters receiver_email , subject and content. Finally, respond with either “Your email has been successfully sent to receiver_email.” or “Failed to send email: error_message.”",
...
export const Triage_Agent = new Agent({
    ...
    instructions:'You are helpful assistant that navigates between Email_Writer agents to provide the best response based on the query.',
